...
| spatial_clustering metric | `adjacency` (shared edges) vs `components` (cluster count) | Both available via `clustering_metric`; adjacency correlates with cost, components tests for decoupling | Metric choice changes the compactness-vs-cost trade-off |
